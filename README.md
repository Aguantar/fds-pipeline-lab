...
> **"TPS 70에서 17,500까지, 어떻게 250배를 개선했는가?"**

이 프로젝트는 단순히 "이상거래 탐지 시스템을 만들었다"가 아닙니다.

**증명하고 싶었던 것:**

| 질문 | 답 |
|------|-----|
| 병목이 어디인지 어떻게 찾았나요? | Connection → 순차처리 → DB I/O, 단계별 측정 |
| 왜 그 기술을 선택했나요? | Redis는 버퍼, n8n은 단순 모니터링에 적합 |
| 한계는 어디까지인가요? | PostgreSQL WAL I/O, 그 이상은 DB 튜닝 필요 |
| 실제 운영은 고려했나요? | SLA 모니터링 + 이상거래 알림까지 구현 |

**결국 이 프로젝트는:**
- ✅ 성능 최적화 **과정**을 설명할 수 있고
- ✅ 기술 선택의 **근거**를 말할 수 있고  
- ✅ 시스템의 **한계**를 알고 개선 방향을 제시할 수 있음
